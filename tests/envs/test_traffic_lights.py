...
class TestGetState(unittest.TestCase):
    ...
    def test_incoming_edges(self):
        ...
        self.env = smart_grid_example(additional_env_params=env_params).env
 
        # center0 assert
        edges = ['right0_0', 'top0_1', 'left1_0', 'bot0_0']
        self.assertEqual(
            sorted(self.env.incoming_edge_ids[0]),
            sorted(edges))

        # center1 assert
        edges = ['right0_1', 'top0_2', 'left1_1', 'bot0_1']
        self.assertEqual(
            sorted(self.env.incoming_edge_ids[1]),
            sorted(edges))

        # center2 assert
        edges = ['right1_0', 'top1_1', 'left2_0', 'bot1_0']
        self.assertEqual(
            sorted(self.env.incoming_edge_ids[2]),
            sorted(edges))

        # center3 assert
        edges = ['right1_1', 'top1_2', 'left2_1', 'bot1_1']
        self.assertEqual(
            sorted(self.env.incoming_edge_ids[3]),
            sorted(edges))
    ...
    def test_outgoing_edges(self):
        ...
        self.assertEqual(
            sorted(self.env.outgoing_edge_ids[0]),
            sorted(edges))

        # center1 assert
        edges = ['left0_1', 'bot0_2', 'right1_1', 'top0_1']
        self.assertEqual(
            sorted(self.env.outgoing_edge_ids[1]),
            sorted(edges))

        # center2 assert
        edges = ['left1_0', 'bot1_1', 'right2_0', 'top1_0']
        self.assertEqual(
            sorted(self.env.outgoing_edge_ids[2]),
            sorted(edges))

        # center3 assert
        edges = ['left1_1', 'bot1_2', 'right2_1', 'top1_1']
        self.assertEqual(
            sorted(self.env.outgoing_edge_ids[3]),
            sorted(edges))
